# Summary

- [AWS index](aws_index.md)

- [AWS](aws.md)

- [AWS Roadmap](aws_roadmap.md)

- [AWS left](aws_left.md)

- [AWS Networking](networking/networking_index.md)
    - [AWS Networking](networking/networking.md)
    - [IPv4 Address Structure, Private IPs, and Public IPs](networking/networking_ip.md)
    - [What is Subnetting?](networking/networking_subnet.md)
    - [What are TCP and UDP?](networking/networking_tcp_udp.md)
    - [Route Tables](networking/networking_route_table.md)
    - [Port](networking/networking_port.md)
    - [Firewalls and Security Concepts](networking/networking_firewall.md)
    - [CIDR Notation](networking/networking_cidr.md)
    - [DNS Resolution Flow](networking/networking_dns.md)
    - [Internet Gateway (IGW) in AWS](networking/networking_igw.md)
    - [Network Address Translation (NAT)](networking/networking_nat.md)
    - [NAT Gateway vs Internet Gateway (IGW)](networking/networking_nat_igw.md)
    - [Security Groups in AWS](networking/networking_sg.md)
    - [Network Access Control Lists (NACLs) in AWS](networking/networking_nacl.md)
    - [NACL vs Security Group (SG)](networking/networking_nacl_sg.md)
    - [Networking doubt](networking/networking_doubt.md)

- [Serverless](serverless/serverless_index.md)
    - [Serverless](serverless/serverless.md)
    - [AWS Lambda](serverless/lambda.md)
    - [AWS API Gateway](serverless/lambda_api_gatway.md)
    - [AWS Pricing](serverless/pricing.md)
    - [Labs](serverless/labs/labs_index.md)
        - [Serverless REST API](serverless/labs/lab1.md)
        - [Event-Driven Processing](serverless/labs/lab2.md)
        - [Asynchronous Messaging](serverless/labs/lab3.md)
        - [Scheduled & Rule-Based Serverless Jobs](serverless/labs/lab4.md)
        - [Secure Serverless Compute](serverless/labs/lab5.md)

- [dev](dev.md)
